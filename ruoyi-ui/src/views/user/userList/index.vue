        ...
        <template slot-scope="scope">
          ...
          <div class="action-buttons">
            ...
            <el-button
             ...
             <el-button
              @click="Reputationenop(scope.row)"
              :style="{
                backgroundColor: '#00CFE8',
                borderColor: '#00CFE8',
                color: '#fff'
...
<!--        <el-button type="primary" @click="submitBalanceForm">确 定</el-button>-->
    ...
    <!--信誉分-->
    <el-dialog :title="ReputationTitle" :visible.sync="ReputationOpen" width="500px" append-to-body>
      <el-form ref="ReputationForm" :model="ReputationForm" :rules="ReputationRules" label-width="80px">
        <el-form-item label="信誉分" prop="creditScore">
          <el-input v-model="ReputationForm.creditScore" placeholder="请输入信誉分" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitReputationForm">确 定</el-button>
        <el-button @click="ReputationCancel">取 消</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      ReputationTitle:'信誉分',
      ReputationOpen: false,
      ReputationForm: {
        uid: '',
        creditScore: '',
      },
      ReputationRules:{
        creditScore: [{ required: true, message: '请输入信誉分', trigger: 'blur' }],
      },
      // 银行卡信息
      bankInfo: {
        bankName: '',
  ...
  methods: {
    ...
    Reputationenop(e){
    this.ReputationOpen = true
    this.ReputationForm.uid = e.uid
    this.ReputationForm.creditScore = e.creditScore
    },
    ReputationCancel(){
    this.ReputationOpen = false
    },
    submitReputationForm()
    {
      updateGradeScore(this.ReputationForm).then(response => {
        this.$modal.msgSuccess("修改成功");
        this.ReputationOpen = false;
        this.getList();
      }).catch(error => {
        ...
        console.log(this.ReputationForm)
    },
    // 发送通知的方法
    submitNotifyForm() {
